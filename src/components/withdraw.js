...
import { withdrawAgency } from "../utils/client";
...
const columns = [
  ...
  {
    field: "amount",
    headerName: "Invested AGENCY",
    width: 90,
    renderCell: (params) => params.value / 1000000 + " AGENCY",
  },
  {
...
const Withdraw = ({ open, close, investments, updateRows }) => {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      const res = await withdrawAgency(state.signingClient, state.address);
      dispatch({
        type: "SET_MESSAGE",
        payload: {
  ...
  return (
    <Modal
      ...
      aria-describedby="Helps new users to get some AGENCY Tokens / JUNO Tokens to be able to test the application."
    >
      <Box sx={style}>
        {loadingWithdraw ? (
          <Grid
