...
import { requestAgency, requestJuno, getAgencyBalance } from "../../utils/client";
...
const StepTwo = ({ changeStep }) => {
  ...
  const init = async () => {
    setLoadingStatus("Getting current AGENCY Balance...");
    const bal = await getAgencyBalance(state.address, chain.rpc);
    ...
    if (state.junoBalance == 0) {
      ...
      const request = async () => {
        ...
        if (balance.amount > 5000) {
          ...
          // .. and continue with AGENCY
          setLoadingStatus("Requesting AGENCY Faucet!");
          executeAgencyFaucet();
        }
      };
      ...
      // Request AGENCY Tokens
      executeAgencyFaucet();
    }
  };

  const executeAgencyFaucet = async () => {
    // Request AGENCY
    requestAgency(state.signingClient, state.address);

    const request = async () => {
      setLoadingStatus("Waiting for AGENCY Balance!");
      const res = await getAgencyBalance(state.address, chain.rpc);
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            <Typography variant="body">
              Your current AGENCY Balance seems to be 0! Since we're still on
